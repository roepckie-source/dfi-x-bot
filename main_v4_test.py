...
def main():
    ...
    # History-Abschnitt voll ausschreiben (ID, Titel & voller Text)
    ...
    btc_change = (
        global_data.get("bitcoin", {}).get("change", 0.0)
        if isinstance(global_data, dict)
        else 0.0
    )
    ...
    eth_change = (
        global_data.get("ethereum", {}).get("change", 0.0)
        if isinstance(global_data, dict)
        else 0.0
    )
    ...
    # 5. Vollständigen Bericht zusammensetzen
    full_report = (
        ...
        f"{daily_insight}\n\n"
        f"📚 {lang_data.get('history', 'DeFiChain Geschichte')}:\n"
        f"{history_block}\n"
        f"-----------------------------------------\n"
